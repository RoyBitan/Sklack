...
const ProposalCreationModal: React.FC<ProposalCreationModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("שגיאה ביצירת הבקשה: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in-up">
      <div className="bg-white w-full sm:w-full sm:max-w-md rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
            ...
            >
              <label className="cursor-pointer w-full h-full flex flex-col items-center justify-center">
                <input
                  type="file"
                  accept="audio/*"
                  capture // Attempt to trigger microphone on mobile
                  onChange={(e) =>
