    ...
    <div className="space-y-8 md:space-y-12 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
        <button
          onClick={() => {
            setStatusFilter((f) =>
              f === TaskStatus.WAITING ? "ALL" : TaskStatus.WAITING
            );
        ...
        <button
          onClick={() => {
            setStatusFilter((f) =>
              f === TaskStatus.IN_PROGRESS ? "ALL" : TaskStatus.IN_PROGRESS
            );
            document.getElementById("task-list")?.scrollIntoView({
              behavior: "smooth",
            });
        ...
        <button
          onClick={() => {
            setStatusFilter((f) =>
              f === TaskStatus.COMPLETED ? "ALL" : TaskStatus.COMPLETED
            );
            document.getElementById("task-list")?.scrollIntoView({
              behavior: "smooth",
            });
      ...
      <div className="flex flex-col xl:flex-row gap-4 md:gap-6 items-center sticky top-16 md:top-28 z-40 bg-[#f8f9fa]/80 backdrop-blur-xl py-4 -my-4">
        ...
        {/* Staff/Admin Toggleable View Logic */}
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={async () => {
                                      if (
                                        window.confirm(
                                          "האם לאשר את הבקשה ולהעביר לטיפול?",
                                        )
                                      ) {
                                  ...
                                  <button
                                    onClick={async () => {
                                      ...
                                      if (response) {
                                        const [newDate, newTime] = response
                                          .split(" ");
                                  ...
                                  <button
                                    onClick={async () => {
                                      if (
                                        window.confirm("האם לבטל את הבקשה?")
                                      ) {
                                        await updateTaskStatus(
                                          task.id,
