...
const PublicOrderStatus: React.FC<PublicOrderStatusProps> = ({ taskId: propTaskId }) => {
    ...
    const getProgressPercentage = () => {
        if (!task) return 0;
        switch (task.status) {
            case TaskStatus.WAITING: return 33;
            case TaskStatus.IN_PROGRESS: return 66;
    ...
    const getStatusLabel = () => {
        if (!task) return '';
        switch (task.status) {
            case TaskStatus.WAITING: return 'ממתין לטיפול';
    ...
    const getStatusIcon = () => {
        ...
        switch (task.status) {
            case TaskStatus.WAITING: return Clock;
            case TaskStatus.IN_PROGRESS: return Wrench;
            case TaskStatus.COMPLETED: return CheckCircle2;
    ...
    const getStatusColor = () => {
        ...
        switch (task.status) {
            case TaskStatus.WAITING: return 'text-orange-500';
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl shadow-2xl p-12 max-w-md text-center">
                    ...
                    <h2 className="text-2xl font-black text-gray-900 mb-2">משימה לא נמצאה</h2>
                    <p className="text-gray-500 font-medium">{error || 'אנא בדוק את הקישור ונסה שוב'}</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8">
            <div className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-3xl shadow-xl p-8 text-center">
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-gray-900 mb-2">מעקב אחר הרכב שלך</h1>
                    <p className="text-gray-500 font-medium">עדכון בזמן אמת על סטטוס הטיפול</p>
                </div>
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div className="flex-1">
                                <div className="bg-[#FFE600] border-4 border-black rounded-[1.2rem] px-5 py-2 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] inline-block mb-2">
                                    <span className="font-mono font-black text-xl tracking-[0.2em]">{vehicle.plate}</span>
                                </div>
                                <p className="text-lg font-bold text-gray-700">{vehicle.model}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Task Info */}
                <div className="bg-white rounded-3xl shadow-xl p-8">
                    <h2 className="text-2xl font-black text-gray-900 mb-4">{task.title}</h2>
                    {task.description && (
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        <div className={`w-14 h-14 ${getStatusColor().replace('text-', 'bg-').replace('500', '100')} ${getStatusColor()} rounded-2xl flex items-center justify-center`}>
                            <StatusIcon size={28} />
                        </div>
                        <div>
                            <p className="text-sm font-bold text-gray-500 uppercase tracking-wider">סטטוס</p>
                            <p className={`text-xl font-black ${getStatusColor()}`}>{getStatusLabel()}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
                            <div
                                className={`h-full transition-all duration-1000 ${task.status === TaskStatus.COMPLETED ? 'bg-green-500' :
                        ...
                        <div className="flex justify-between pt-4">
                            <div className={`text-center ${task.status === TaskStatus.WAITING || task.status === TaskStatus.IN_PROGRESS || task.status === TaskStatus.COMPLETED ? 'opacity-100' : 'opacity-30'}`}>
                                <div className={`w-3 h-3 rounded-full mx-auto mb-2 ${task.status === TaskStatus.WAITING || task.status === TaskStatus.IN_PROGRESS || task.status === TaskStatus.COMPLETED ? 'bg-orange-500' : 'bg-gray-300'}`}></div>
                                <p className="text-xs font-bold text-gray-500">התקבל</p>
                            </div>
                            <div className={`text-center ${task.status === TaskStatus.IN_PROGRESS || task.status === TaskStatus.COMPLETED ? 'opacity-100' : 'opacity-30'}`}>
                                <div className={`w-3 h-3 rounded-full mx-auto mb-2 ${task.status === TaskStatus.IN_PROGRESS || task.status === TaskStatus.COMPLETED ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                                <p className="text-xs font-bold text-gray-500">בטיפול</p>
                            </div>
                            <div className={`text-center ${task.status === TaskStatus.COMPLETED ? 'opacity-100' : 'opacity-30'}`}>
                                <div className={`w-3 h-3 rounded-full mx-auto mb-2 ${task.status === TaskStatus.COMPLETED ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        ...
                        {task.status === TaskStatus.COMPLETED && task.updated_at && (
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-black text-green-600">
                                    {new Date(task.updated_at).toLocaleDateString('he-IL')} {new Date(task.updated_at).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}
                                </span>
                            </div>
                        )}
                    </div>
                ...
                <div className="bg-gradient-to-r from-black to-gray-800 rounded-3xl shadow-xl p-8 text-white text-center">
                    ...
                    <p className="text-white/80 font-medium mb-6">צור קשר עם המוסך לפרטים נוספים</p>
                    <a
                ...
                <div className="text-center py-4">
                    <p className="text-xs text-gray-400 font-bold">Powered by Sklack Garage OS</p>
                </div>
            </div>
        </div>
    );
