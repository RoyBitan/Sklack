...
import { Task, TaskStatus, UserRole, Profile } from '../types';
import {
    Clock,
    Wrench,
    AlertCircle,
    ...
    if (!task) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
                ...
                <h2 className="text-xl font-black text-gray-900 mb-2">המשימה לא נמצאה</h2>
                <button
                    onClick={() => {
    ...
    return (
        <div className="min-h-screen bg-[#f8f9fa] flex flex-col">
                    ...
                    <div className="text-start">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <div className="bg-[#FFE600] border-2 border-black rounded-lg px-2 py-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                <span className="font-mono font-black text-xs tracking-widest">{formatLicensePlate(task.vehicle?.plate || '')}</span>
                            </div>
                            <span className="text-xs font-bold text-gray-400">|</span>
                            <span className="text-xs font-black text-gray-500">{task.vehicle?.model}</span>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto w-full max-w-3xl mx-auto p-5 md:p-8 space-y-6 pb-24 native-scroll">
                ...
                <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <div className="flex items-center gap-4 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-gray-400 uppercase mb-0.5">צוות בביצוע</div>
                                <div className="font-black text-gray-900 text-base">
                                    {assignedWorkers.length > 0 ? assignedWorkers.map(w => w.full_name).join(', ') : 'טרם שויך'}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-5 rounded-[1.5rem] shadow-sm border border-gray-100 text-start group hover:border-purple-200 transition-colors">
                            <div className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center text-purple-600 mb-3"><UserIcon size={20} /></div>
                            <div className="text-[10px] font-black text-gray-400 uppercase mb-1 tracking-wider">שם המנוי</div>
                            <div className="font-black text-gray-900 text-lg">
                                {task.vehicle?.owner?.full_name || (task.metadata as any)?.ownerName || '---'}
                            </div>
                        </div>
                        <a
                            href={`tel:${task.vehicle?.owner?.phone || (task.metadata as any)?.ownerPhone}`}
                ...
                <div className="space-y-4">
                    <div className="text-[11px] font-black text-gray-400 uppercase tracking-[0.15em] px-2 text-start">מפרט רכב מלא</div>
                    <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-gray-100">
                        <div className="grid grid-cols-2 gap-6 text-start">
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">דגם ושנה</div>
                                <div className="font-black text-gray-900 text-base">{task.vehicle?.model} {task.vehicle?.year ? `(${task.vehicle.year})` : ''}</div>
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">צבע וגימור</div>
                                <div className="font-black text-gray-900 text-base">{task.vehicle?.color || '---'}</div>
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">סוג דלק</div>
                                <div className="font-black text-gray-900 text-base">{task.vehicle?.fuel_type || '---'}</div>
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">קודנית / נעילה</div>
                                <div className="font-black text-red-600 font-mono tracking-widest text-base">{task.immobilizer_code || task.vehicle?.kodanit || '---'}</div>
                            </div>
                            <div className="col-span-2 pt-4 border-t border-gray-50 space-y-1">
                                ...
                                <div className="font-black text-gray-900 font-mono text-sm uppercase break-all">{task.vehicle?.vin || '---'}</div>
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">דגם מנוע</div>
                                <div className="font-black text-gray-900 font-mono uppercase text-sm">{task.vehicle?.engine_model || '---'}</div>
                            </div>
                            <div className="space-y-1">
                                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">טסט בתוקף עד</div>
                                <div className="font-black text-gray-900 text-base">{task.vehicle?.registration_valid_until || '---'}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="text-[11px] font-black text-gray-400 uppercase tracking-[0.15em] px-2 text-start">תיאור העבודה והערות</div>
                    <div className="bg-purple-900 text-white p-7 md:p-10 rounded-[2rem] shadow-xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-5">
                            ...
                            <p className="text-xl font-black leading-relaxed italic text-start">
                                "{task.description || (task.metadata as any)?.faultDescription || 'אין הערות נוספות למשימה זו.'}"
                            </p>
                            <div className="pt-4 border-t border-white/10 flex items-center gap-3">
                                <Clock size={16} className="text-purple-300" />
                                <span className="text-[10px] font-black uppercase tracking-widest opacity-60">נפתח ב: {new Date(task.created_at).toLocaleString('he-IL')}</span>
                            </div>
                        </div>
                    </div>
                </div>
