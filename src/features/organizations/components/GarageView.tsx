...
const GarageView: React.FC = () => {
  ...
  const { vehicles, loading: dataLoading, refreshData, removeVehicle } =
    useData();
  ...
  if (dataLoading && activeTab === "VEHICLES") {
    ...
    <div className="space-y-6 animate-fade-in-up pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex-1 w-full md:w-auto">
          {isEditingName
            ? (
              <div className="flex flex-col md:flex-row items-start md:items-center gap-4 animate-fade-in w-full">
                <input
                  type="text"
                  className="input-premium h-14 md:h-16 text-xl px-6 flex-1 min-w-[300px]"
                  value={newGarageName}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      ...
                      setSaving(true);
                      try {
                        await updateOrganization({ name: newGarageName });
                    ...
                    disabled={saving}
                    className="bg-black text-white px-6 h-14 md:h-16 rounded-xl font-black text-sm hover:bg-gray-800 transition-all flex items-center justify-center gap-2"
                  >
                    {saving
                      ? (
                  ...
                  <button
                    onClick={() => {
                      setIsEditingName(false);
                      setNewGarageName(profile?.organization?.name || "");
                    }}
                    className="bg-gray-100 text-gray-500 px-6 h-14 md:h-16 rounded-xl font-black text-sm hover:bg-gray-200 transition-all"
                  >
                    ביטול
                  </button>
                </div>
              ...
              <div className="group flex items-center gap-4">
                ...
                {profile?.role === UserRole.SUPER_MANAGER && (
                  <button
                    ...
                    className="p-2 hover:bg-gray-100 rounded-lg text-gray-300 hover:text-black transition-all"
                    title="ערוך שם מוסך"
                  >
                    <Edit2 size={18} />
                  </button>
      ...
      <div className="bg-white p-1.5 rounded-2xl flex shadow-sm border border-gray-100 overflow-x-auto">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm transition-all whitespace-nowrap px-4 ${
            activeTab === "VEHICLES"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredVehicles.map((v: Vehicle) => (
                <div
                  key={v.id}
                  className="group border-2 border-gray-50 hover:border-black rounded-[2rem] p-6 transition-all duration-300 hover:shadow-lg relative bg-gray-50/50 hover:bg-white"
                >
        ...
        {activeTab === "TEAM" && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-black text-gray-900 px-2">
              ...
              {loadingMembers
                ...
                : (
                  teamMembers.map((member: Profile) => (
                    <div
                      key={member.id}
                      className="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors border border-gray-100"
                    >
        ...
        {activeTab === "CUSTOMERS" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {customers.map((member: Profile) => (
                <div
                  key={member.id}
                  className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 bg-white hover:shadow-md transition-all"
                >
