...
const EditGarageCodeModal: React.FC<EditGarageCodeModalProps> = (
  { currentCode, onClose, onSuccess },
) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      const message = err instanceof Error ? err.message : "שגיאה בעדכון הקוד";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-fade-in-up">
      <div
        ref={formRef}
        className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden relative max-h-[90vh] overflow-y-auto"
      >
