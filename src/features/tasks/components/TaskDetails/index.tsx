...
import { Profile, TaskStatus, UserRole } from "@/types";

/**
 * TaskDetails - Detailed view for a single task
 * Refactored for complete type safety and consistency.
 */
const TaskDetails: React.FC = () => {
  ...
  // Note: updateProposal is still in DataContext, but we might want to move it to proposals service
  // For now, let's keep it if we can't easily move it, or use a direct supabase call.
  // Actually, let's keep it as is for now or use supabase directly to decouple.
    ...
    const fetchWorkers = async () => {
      if (task?.assigned_to && task.assigned_to.length > 0) {
        const { data } = await supabase
          .from("profiles")
          .select(
            "id, full_name, role, phone, org_id, membership_status, created_at",
          )
  ...
  const updateProposal = async (
    ...
    try {
      ...
      // refresh tasks via context if possible, or reload
    } catch (e) {
      console.error(e);
      toast.error("שגיאה בעדכון ההצעה");
    }
  };
  ...
  const ownerPhone = getTaskOwnerPhone(task);
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className="bg-[#FFE600] border-2 border-black rounded-lg px-2 py-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                <span className="font-mono font-black text-xs tracking-widest">
                  {formatLicensePlate(
                    task.vehicle?.plate || metadata?.vehiclePlate || "",
                  )}
                </span>
              </div>
              <span className="text-xs font-bold text-gray-400">|</span>
              <span className="text-xs font-black text-gray-500">
                {task.vehicle?.model || metadata?.vehicleModel || "דגם לא ידוע"}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white p-6 rounded-[1.5rem] shadow-sm border border-gray-100">
            <div className="grid grid-cols-2 gap-6 text-start">
              <div className="space-y-1">
                ...
                <div className="font-black text-gray-900 text-base">
                  {task.vehicle?.model || metadata?.vehicleModel || "---"}{" "}
                  {task.vehicle?.year || metadata?.vehicleYear
                    ? `(${task.vehicle?.year || metadata?.vehicleYear})`
                    : ""}
                </div>
              </div>
              <div className="space-y-1">
                ...
                <div className="font-black text-gray-900 text-base">
                  {task.vehicle?.color || metadata?.vehicleColor || "---"}
                </div>
              </div>
              <div className="space-y-1">
                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
                ...
                <div className="font-black text-gray-900 text-base">
                  {task.vehicle?.fuel_type || metadata?.fuelType || "---"}
                </div>
              </div>
              <div className="space-y-1">
                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
              ...
              <div className="space-y-1">
                ...
                <div className="font-black text-gray-900 font-mono uppercase text-sm">
                  {task.vehicle?.engine_model || metadata?.engineModel || "---"}
                </div>
              </div>
              <div className="space-y-1">
                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
          ...
          <div className="bg-purple-900 text-white p-7 md:p-10 rounded-[2rem] shadow-xl relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-5">
              ...
              <p className="text-xl font-black leading-relaxed italic text-start">
                "{faultDescription || "אין הערות נוספות למשימה זו."}"
              </p>
