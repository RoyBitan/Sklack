...
const EditTaskModal: React.FC<EditTaskModalProps> = ({ task, onClose }) => {
  ...
  const [teamMembers, setTeamMembers] = useState<import("@/types").Profile[]>(
    [],
  );
  ...
  }, [showAssignment, fetchTeamMembers, teamMembers.length]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in-up">
      <div
        ref={scrollRef}
        className="bg-white w-full h-full sm:h-auto sm:w-full sm:max-w-2xl sm:rounded-[2rem] shadow-2xl flex flex-col transition-all duration-300"
      >
