...
const TaskChat: React.FC<TaskChatProps> = ({ taskId, isInternalOnly }) => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Staff/Managers can always chat, Customers check subscription
    const canSend = profile?.role !== UserRole.CUSTOMER || canUseChat;
    if (!canSend) return;
      ...
      >
        ...
        <button
          ...
          disabled={!newMessage.trim() ||
            (!canUseChat && profile?.role === UserRole.CUSTOMER)}
